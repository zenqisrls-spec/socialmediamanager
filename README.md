...
.\.venv\Scripts\Activate.ps1
python -m pip install --upgrade pip setuptools wheel
pip install --index-url https://pypi.org/simple --only-binary=:all: "pydantic==2.12.4"
pip install -r requirements.txt
```
...
Se compare ancora `pydantic-core==2.33.2`, il tuo indice pacchetti è probabilmente incoerente (ha `pydantic-core` nuovi ma un `pydantic` vecchio): usa i comandi con `--index-url https://pypi.org/simple` qui sopra.
...
```powershell
python -m pip install --upgrade pip setuptools wheel
pip install --index-url https://pypi.org/simple "pydantic==2.12.4"
pip install --index-url https://pypi.org/simple -r requirements.txt
```
